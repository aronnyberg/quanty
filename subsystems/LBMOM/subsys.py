...
class Lbmom:
    ...
    def extend_historicals(self, instruments, historical_data):
        ...
        for inst in instruments:
            ...
            for pair in self.pairs:
                ...
                # below created due to performance warning, also altered indicators_cal.py to output numpy arrays instead of df's, way slower altogether!
                ...
                #ema_diff_dict = {"{} ema{}".format(inst, str(pair)):indicators_cal.ema_series(historical_data["{} close".format(inst)], n=pair[0]) - \
                #    indicators_cal.ema_series(historical_data["{} close".format(inst)], n=pair[1])}
                #historical_data = pd.concat([historical_data, pd.DataFrame(ema_diff_dict)], axis=1)
    ...
    def run_simulation(self, historical_data, historical_strategy_df, debug=False, use_disk=False):
        ...
        """
        logger.debug(str(datetime.datetime.now())+' started lbmom')
        ...
        if not use_disk:
            ...
            logger.debug(str(datetime.datetime.now())+f'hist_strat_index: {historical_strategy_df.index[:2]}')
            ...
            logger.debug(str(datetime.datetime.now())+f' lbmom strat index : {historical_strategy_df.index[-1]}')
            historical_data = historical_data[historical_data.index > historical_strategy_df.index[-1]]
            logger.debug(str(datetime.datetime.now())+f' lbmom hist_data index : {historical_data.index[0]}')
            historical_data = self.extend_historicals(instruments=instruments, historical_data=historical_data)
            portfolio_df = pd.DataFrame(index=historical_data.index).reset_index()
            ...
            #is_halted = lambda inst, date: not (~historical_data[:date].tail(3)["{} active".format(inst)]).any()
